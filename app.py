...
class LinkedInCertArchitectSuite(ctk.CTk if hasattr(ctk, 'CTk') else tk.Tk):
    ...
    def build_standalone_html_portfolio(self):
        ...
        template_paths = [
            ...
            os.path.join(APP_DIR, "assets", "template.html")
        ]
        template = ""
        ...
        if template:
            return template.replace("__PROFILE_OWNER__", owner_name).replace("__PROFILE_HEADLINE__", headline).replace("__CERTIFICATES_JSON__", certs_json)
            
        # Minimal fallback if template file is missing
        return f"<!DOCTYPE html><html><head><title>{owner_name}</title></head><body><h1>{owner_name}</h1><p>{headline}</p></body></html>"
